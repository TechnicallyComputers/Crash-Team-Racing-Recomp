...
#
# Writes: dist/ctr-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix ctr \
  --exe-name Crash_Team_Racing__Recompiled \
  --display-name "Crash Team Racing  Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Crash Team Racing disc" \
